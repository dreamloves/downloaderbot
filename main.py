...
BOT_TOKEN = "6338695339:AAGOrn_9WovtN0NWV1vZVIMwKiFYgFx_XV4,6836162892:AAEUPfCOJGNUPZPkzFfpPlQY79T9HlPaGno"
AUTH_USERS = 6462391456
sudo_users = [5666581338,6462391456,5684410709,1002123381691]
bot = Client(
    "bot",
    bot_token=BOT_TOKEN,
    api_id=API_ID,
            ...
            try:
                ...
                if cmd == "pdf" or ".pdf" in url or ".pdf" in name:
                    try:
                        ...
                        await prog.delete(True)
                        time.sleep(1)
                        reply = await m.reply_text(f"Haxzor")
                        time.sleep(1)
                        start_time = time.time()
...
async def account_login(bot: Client, m: Message):
    ...
    editable = await m.reply_text("**Pankaj**")
    input0: Message = await bot.listen(editable.chat.id)
    raw_text0 = input0.text
